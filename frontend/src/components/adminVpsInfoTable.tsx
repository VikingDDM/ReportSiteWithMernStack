...
import { useGetAllVpsInfoQuery } from '../redux/api/vpsInfoApi';
import AdminVpsInfoEditModal from './adminVpsInfoEditModal';
...
function AdminVpsInfoTable() {
   
    const { isLoading, isError, error, data: vpsinfo } = useGetAllVpsInfoQuery();
    const [page, setPage] = React.useState(0);
    const [rowsPerPage, setRowsPerPage] = React.useState(5);
    const [dataid, setDataid] = React.useState("");
    const [dataUsername, setDataUsername] = React.useState("");
    const [dataVpsPassword, setDataVpsPassword] = React.useState("");
    const [dataVpsUrl, setDataVpsUrl] = React.useState("");
    const [show, setShow] = React.useState(false);
    // Avoid a layout jump when reaching the last page with empty rows.
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - vpsinfo?.length) : 0;

    const handleShow = (data: any) => {setShow(true); 
                                       setDataid(data._id); 
                                       setDataUsername(data.username);
                                       setDataVpsPassword(data.vpsPassword);
                                       setDataVpsUrl(data.vpsUrl);} 
    ...
    return (
      <TableContainer component={Paper} style={{marginTop:"30px"}} >
        <Table className='borderTable' sx={{ Width: 500 }} aria-label="custom pagination table" >
          ...
          <TableBody>
            {(rowsPerPage > 0
              ? vpsinfo?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              : vpsinfo
            )?.map((row:any, key: any) => (
        ...
        <AdminVpsInfoEditModal 
                  modalShow = {show} 
                  handleModalClose={handleClose}   
                  vpsInfo_id = {dataid}
                  defaultValueA = {dataUsername} 
                  defaultValueB = {dataVpsPassword}
                  defaultValueC = {dataVpsUrl} />
      </TableContainer>
    );
  }

  export default AdminVpsInfoTable;
